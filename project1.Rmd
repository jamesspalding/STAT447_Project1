...
First, we will run a right-tailed two-sample simulated permutation test to see if the mean age at which women are diagnosed with PD is statistically greater than men. The hypothesis is given by:
...
The Siegel-Tukey test returned a test statistic of 32, and a corresponding p-value of 0.00695. Therefore, we can say that there is significant evidence that there is a difference in variance between genders.
...
Due to the variables not being normal, we used Spearman’s $\rho$ coefficient to identify correlation. We then plotted the variables where $\rho > .7$ and discovered that jitter and shimmer were highly correlated, as well as HNR and shimmer. 