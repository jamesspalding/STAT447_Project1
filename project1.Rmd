...
The Dobs from the two-sample simulated permutation test is 3.6 which you can see displayed above in *Figure 3*, and the test gives a corresponding p-val of around 0.084. Therefore we fail to reject $H_0$ at the .05 level. We can conclude that we do not have enough evidence to say that women are diagnosed with PD at a later age than men.
...
```{r, echo=FALSE, fig.cap="Variange of Age Diagnosed with PD Between Sexes Permutation Test", fig.align='center'}
hist(Dperm, main = "") %>%
  abline(v=Dobs, col="blue", lwd=2)
```

The two-sample simulated permutation test gives a Dobs of 85.54 which you can see above as the blue line in *Figure 4*, and a corresponding p-val of around 0.003. We can then reject $H_0$ at the .05 level. We can then conclude with this test that we have significant evidence to suggest that the variance for when women are diagnosed with PD is greater than it is for men.
