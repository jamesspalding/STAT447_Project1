...
Firstly, we looked further into the data to discover that none of the voice measurement data was able to be assumed normally distributed, so we would then have to run non-parametric statistics to further analyze. We were a little surprised by running a two-sample simulated permutation test to discover that the minimum, maximum, and average of each vocal fundamental frequency, one variable from the jitter ratings, and the diagnosis status all significantly differ between genders. After further discovery, we can conclude with the same test that females have a higher mean vocal fundamental frequency than the males. Also, that males had a statistically significant higher mean jitter rating, and that they are more frequently diagnosed than females are. We then ran a McNemar’s test on the number of diagnosis for men and women to support that there is a difference in the amount of diagnoses between sexes.

We then further discovered with the same permutation testing that there is a significant difference between the means for every variable other than vocal fundamental frequency between the patients aged 45-65 and 66-85. Further analysis showed that the 66-85 age group has statistically significant higher mean values for all variables except NHR and average vocal fundamental frequency. NHR was the only variable found to have a statistically higher mean value for the 45-65 age group. 

Due to the variables not being normal, we used Spearman’s $\rho$ coefficient to identify correlation. We then plotted the variables where $\rho > .7$ and discovered that jitter and shimmer were highly correlated, as well as HNR and shimmer. Also, it is worth noting that the older age had higher results for every one of the measure displayed above.

We then were interested in testing if there is differences between the age that women and men were likely to be diagnosed with PD. A two-sample simulated permutation test showed that we did not have enough evidence to suggest anything on that front. After finding no difference in the age that men and women are diagnosed, we were interested if there was any difference in the variance that either gender was diagnosed. Using a Siegel Tukey test with an $\alpha$ of .05 resulted where we were able to reject the null, and state that we had significant evidence to suggest that the variances were different between genders.
...
The last thing we looked into, is what measures from the voice data we were given is statistically different between patients with PD to those without. We used a two-sample simulated permutation two-tailed test with an $\alpha$ of .05 for this. We found that there was the most differences in the variables between patients with PD to those without for the means. There was then only one variable that had a statistically different variance, which was Flo. And then only Flo and Fhi were found to have statistically different medians. From this, we can conclude that Flo is the best indicator to be the only variable that has a statistically different mean, variance, and median between patients with PD to those without. 
