...
```{r, include=FALSE}
=======
got to test for a difference in the means probably for there being a difference in the age diagnosed.

We discovered earlier how the number of men and women that are diagnosed with PD is statistically different. We are now interested if there is a statistical difference between the age at which men and women are diagnosed with PD, along with if there is a difference in the variability between when men and women are diagnosed. For testing for a difference between men and women in the variability of diagnosis we will use a two-sided Siegel Tukey Test with an $\alpha$ level of .05.
...
```{r}
>>>>>>> d6a56ac9cbe23b945c1f515b3baa9c09548db37a
